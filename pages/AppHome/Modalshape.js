...
function Modalshape() {
    ...
    const [listshape, setListshape] = useState([
     ...
     const {setvisibleshape}=useContext(ShapeContext)
     const {changevalueshape}=useContext(ShapeContext)
    ...
    return (
        <>
            ...
            <View style={style.modal}>
                <AntDesign name="close" style={style.icon} onPress={()=>setvisibleshape()}></AntDesign>
                <View style={style.main}>
                    {
                        listshape.map((item, index) => (
                            <TouchableOpacity style={style.choice} onPress={()=>changevalueshape(item.text)}>
